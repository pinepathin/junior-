...
export class ViewPostComponent implements OnInit {
  ...
  addComment() {
    ...
    this.commentService
      .createComment(
        { ...this.validateForm.value, postId: this.postId },
        this.image
      )
      .subscribe((res) => {
    ...
    if (!this.post.comments.last) {
      ...
      this.commentService
        .getCommentByPostId(this.postId, {
          page: (this.post?.comments?.number ?? -1) + 1,
        })
        .subscribe((newPage) => {
  ...
  toggleVote(commentIndex: number, commentId: number) {
    ...
    const hasVote: boolean = this.isCurrentUserHasVote(
      this.post.comments.content[commentIndex]?.votes ?? []
    );
    this.isLoading = true;
    if (!hasVote) {
      this.commentService.setVote(commentId).subscribe(() => {
        this.post.comments.content[commentIndex].votes = [
          { userId: StorageService.getCurrentUser().id, commentId: commentId },
      ...
      this.commentService.clearVote(commentId).subscribe(() => {
        this.post.comments.content[commentIndex].votes = Array.from(
          this.post.comments.content[commentIndex].votes
        ).filter((voteObj: any) => {
          return !StorageService.isCurrentUser(voteObj.userId);
        });
